...
function run(command, callback = noop) {
	const [entry, ...commandArgs] = (command instanceof Array ? command : command.split(' ')).filter(v => v)
	const sCommand = entry + ' ' + commandArgs.join(' ')
	console.log('$', sCommand)  // eslint-disable-line no-console
	...
	return new Promise((resolve, reject) => {
		...
		cmd.stderr.on('data', data => {
			...
			console.log('ERR', text)  // eslint-disable-line no-console
			const output = {
				text: text,
				type: 'stderr',
			}
