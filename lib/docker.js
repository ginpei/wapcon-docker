...
async function pull(image, tag, callback) {
	if (!image) {
		...
		for (const line of text.split('\n')) {
			if (rxFirstLine.test(line)) {
				...
				if (id) {
					...
					layers.set(id, progressStatus)
				}
			}

			status.numAll = layers.size
			status.numCompleted = [...layers.values()].filter(v => v === status.COMPLETE).length
